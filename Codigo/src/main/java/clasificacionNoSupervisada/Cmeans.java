...
public class Cmeans {
    ...
    public void elegirClusters(){ //aleatorios sin repetirse
        ...
        this.clusters.add(0, new PatronRepresentativo("closter0", this.patrones.get(primerPos).getVector()));//agrega el patron en la posicion en las clusters
        ...
        for(int i=1; i<this.numClases; i++){ //genera el resto de posiciones
            ...
            posMedias.add(num);
            this.clusters.add(new PatronRepresentativo("closter" + i, this.patrones.get(num).getVector()));
        } 
    ...
    public ArrayList<PatronRepresentativo> ajustesClusters(){
        ...
        auxClusters.add(new PatronRepresentativo(this.patrones.get(0)));
           ...
           int posicion = buscarClase(aux, auxClusters); //busca si su clase ya existe
             if(posicion!=-1)//si exite agrega ese elemento al representativo correspondiente
                auxClusters.get(posicion).acumular(aux);
                 ...
                 auxClusters.add(new PatronRepresentativo(aux));
        }
    ...
    public int buscarClase(Patron aux, ArrayList<PatronRepresentativo> auxClusters){
        ...
        boolean encontrado = false; //bandera para saber si exite la clase representativa de la nueva instancia
           for(int i=0; i<auxClusters.size(); i++){ //recorre los representativos
               if(auxClusters.get(i).getClase().equals(aux.getClase())){ //compara la clase de la nueva instancia con la clase de los representativos
                   encontrado=true;
                   posicion=i;
                   break;
               }
